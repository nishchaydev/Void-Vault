...
fn verify_zones(
    ...
    info!(
        "Starting zone-based verification ({} zones, {} sectors)",
        zones.len(),
        zone_total_sectors
    );
    ...
    for (zone_start, zone_count) in &zones {
        ...
        while read_in_zone < *zone_count {
            ...
            if is_random {
                crate::sanitize::patterns::fill_buffer(
                    &mut expected_buf[..bytes_to_read],
                    expected_pattern,
                );
            }
            ...
            match result {
                Ok(()) => {
                    ...
                    if !is_random {
                        ...
                        // Random verification: use entropy analysis instead of
                        // impossible pattern matching. Verify that readback data
                        // has high entropy (≥ 7.90 bits/byte), confirming it was
                        // overwritten with random data and not left as original content.
                        let entropy_result = crate::verify::entropy::analyze(
                            &read_buf[..bytes_to_read],
                            true, // is_random_method
                        );
                        match entropy_result.verdict {
                            crate::verify::entropy::EntropyVerdict::Pass
                            | crate::verify::entropy::EntropyVerdict::Warning => {
                                sectors_verified += sectors_to_read;
                            }
                            crate::verify::entropy::EntropyVerdict::Fail => {
                                sectors_failed += sectors_to_read;
                                if first_failure_sector.is_none() {
                                    first_failure_sector = Some(zone_start + read_in_zone);
                                }
                                warn!(
                                    "Random verification FAILED at sector {}: entropy {:.4} (expected >= 7.90)",
                                    zone_start + read_in_zone,
        ...
        match result {
            Ok(()) => {
                ...
                if !is_random && check_mismatch {
                    ...
                    if is_random {
                        // Random verification: use entropy analysis to confirm
                        // data was actually overwritten with random bytes.
                        let entropy_result = crate::verify::entropy::analyze(
                            &read_buf[..bytes_to_read],
                            true,
                        );
