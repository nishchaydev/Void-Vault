...
pub fn reinitialize_and_format_disk(
    ...
    fs::write(&temp_file, script_content).with_context(|| {
        format!(
            "Failed to create temporary diskpart script at {:?}",
            temp_file
        )
    })?;
    ...
    if !output.status.success() || stdout_str.contains("DiskPart has encountered an error") {
        let err_detail = if !stderr_str.trim().is_empty() {
            ...
            stdout_str
                .lines()
                .filter(|l| {
                    l.contains("Error") || l.contains("error") || l.contains("Virtual Disk Service")
                })
                .collect::<Vec<_>>()
                .join("\n")
        };
