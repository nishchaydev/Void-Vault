...
impl SanitizeMethod {
    pub fn pass_count(&self) -> usize {
        match self {
            ...
            | Self::HmgIs5Baseline
            | Self::Random1Pass => 1,
            Self::NistPurge => 1, // NIST 800-88 Purge: single random overwrite; verification is a separate read-back step
            Self::GostR50739 => 2,
            Self::Dod3Pass
            | Self::HmgIs5Enhanced
            | Self::Afssi5020
...
pub fn get_pattern(pass_index: usize, method: SanitizeMethod) -> FillPattern {
    match method {
        SanitizeMethod::FastWipe
        | SanitizeMethod::SmartSecure
        | SanitizeMethod::NistClear
        | SanitizeMethod::HmgIs5Baseline => FillPattern::Fixed(0x00),
...
/// Uses fast chunk copying and xoroshiro PRNG to achieve multi-GB/s filling speeds.
pub fn fill_buffer(buf: &mut [u8], pattern: &FillPattern) {
    match pattern {
        ...
        FillPattern::Random => {
            use rand::RngCore;
            // Cryptographically seed a fast 64-bit generator once per buffer
            let mut seed = [0u8; 16];
            rand::thread_rng().fill_bytes(&mut seed);
            let mut s0 = u64::from_le_bytes(seed[0..8].try_into().unwrap_or([0x5A; 8]));
            let mut s1 = u64::from_le_bytes(seed[8..16].try_into().unwrap_or([0xA5; 8]));
            if s0 == 0 && s1 == 0 {
                s0 = 0x853c49e6748fea9b;
                s1 = 0xda3e39cb94b95bdb;
            }

            // Word-aligned 8-byte streaming fill (exceeds 4 GB/s)
        ...
        FillPattern::ThreeByteRepeating(a, b, c) => {
            ...
            if buf.len() <= 24 {
                ...
                // Pre-fill a 24-byte block (24 is LCM of 3 and 8)
                let mut chunk = [0u8; 24];
                for i in 0..24 {
                    chunk[i] = pattern[i % 3];
                }
                let mut filled = 0;
                while filled + 24 <= buf.len() {
                    buf[filled..filled + 24].copy_from_slice(&chunk);
                    filled += 24;
                }
                while filled < buf.len() {
                    buf[filled] = pattern[filled % 3];
                    filled += 1;
                }
            }
        }
