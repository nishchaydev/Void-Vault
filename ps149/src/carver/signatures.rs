...
pub fn all_signatures() -> Vec<FileSignature> {
    vec![
        ...
        FileSignature {
            ...
            header: &[0x66, 0x74, 0x79, 0x70, 0x68, 0x65, 0x69, 0x63], // ftypheic
            footer: None,
            max_size: 50 * MB,
            min_size: 16,
            category: FileCategory::Image,
        ...
        FileSignature {
            ...
            header: &[0x66, 0x74, 0x79, 0x70, 0x61, 0x76, 0x69, 0x66], // ftypavif
            footer: None,
            max_size: 50 * MB,
            min_size: 16,
            category: FileCategory::Image,
        ...
        FileSignature {
            ...
            header: &[0x45, 0x6C, 0x66, 0x46, 0x69, 0x6C, 0x65, 0x00], // ElfFile\0
            footer: None,
            max_size: 500 * MB,
            min_size: 4096,
            category: FileCategory::Forensic,
        },
        FileSignature {
            name: "Windows Registry Hive",
            extension: "dat",
            mime_type: "application/x-ms-regf",
            header: &[0x72, 0x65, 0x67, 0x66], // regf
            footer: None,
            max_size: 500 * MB,
            min_size: 4096,
            category: FileCategory::Forensic,
        ...
        FileSignature {
            name: "Windows Prefetch (Compressed MAM)",
            extension: "pf",
            mime_type: "application/x-ms-prefetch",
            header: &[0x4D, 0x41, 0x4D, 0x04], // MAM\x04
            footer: None,
            max_size: 10 * MB,
            min_size: 8,
            category: FileCategory::Forensic,
        ...
        FileSignature {
            ...
            header: &[0x52, 0x61, 0x72, 0x21, 0x1A, 0x07, 0x00], // Rar!\x1a\x07\x00
            footer: None,
            max_size: 10 * GB,
            min_size: 20,
            category: FileCategory::Archive,
        ...
        FileSignature {
            ...
            extension: "rar",
            mime_type: "application/x-rar-compressed",
            header: &[0x52, 0x61, 0x72, 0x21, 0x1A, 0x07, 0x01, 0x00], // Rar!\x1a\x07\x01\x00
            footer: None,
            max_size: 10 * GB,
            min_size: 20,
            category: FileCategory::Archive,
        ...
        FileSignature {
            ...
            max_size: 50 * GB,
            min_size: 512,
            category: FileCategory::Filesystem,
        },
        FileSignature {
            name: "VMware Disk (VMDK)",
            extension: "vmdk",
            mime_type: "application/x-vmdk",
            header: &[0x4B, 0x44, 0x4D, 0x56], // KDMV
            footer: None,
            max_size: 50 * GB,
            min_size: 512,
            category: FileCategory::Filesystem,
            ...
            if &buf[..sig.header.len()] == sig.header {
                ...
                if sig.header == &[0x52, 0x49, 0x46, 0x46] && buf.len() >= 12 {
                    ...
                    match sig.extension {
                        ...
                        "avi" if subtype != b"AVI " && subtype != b"AVIX" => continue,
...
mod tests {
    ...
    #[test]
    fn test_evtx_header_match() {
        let sigs = all_signatures();
        let evtx_data = b"ElfFile\0some_chunk_data";
        let matched = match_header(evtx_data, &sigs);
        assert!(matched.is_some());
        assert_eq!(matched.unwrap().extension, "evtx");
        assert_eq!(matched.unwrap().category, FileCategory::Forensic);
    }
