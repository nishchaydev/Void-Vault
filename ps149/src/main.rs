...
fn main() -> Result<()> {
    ...
    if args.iter().any(|a| a == "--server" || a == "server") {
        let port: u16 = args.windows(2)
            ...
            .unwrap_or(5001);
        println!("{}", format!("🚀 Forensic GUI API server running at http://127.0.0.1:{}", port).green().bold());
        return server::run_api_server(port, running);
    }
    ...
    if groq_client.is_some() {
        println!(
            "  {}",
            "AI Features Enabled (Groq API detected)".bright_magenta()
        );
    }
...
fn print_main_menu() {
    ...
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "Main Menu — Forensics & Secure Sanitization Suite"
            .bold()
            .bright_white(),
        "    │".bright_cyan()
    );
    ...
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "[1]  List Connected Storage Devices".white(),
        "               │".bright_cyan()
    );
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "[2]  Erase an Entire Drive (Module 1)".white(),
        "                   │".bright_cyan()
    );
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "[3]  Secure File & Folder Shredder (Module 2)"
            .bright_green()
            .bold(),
        "   │".bright_cyan()
    );
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "[4]  File Recovery & Deep Carving (Module 3)"
            .bright_yellow()
            .bold(),
        "    │".bright_cyan()
    );
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "[5]  View Erasure History & Audit Reports".white(),
        "             │".bright_cyan()
    );
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "[6]  Settings & Info".white(),
        "                                │".bright_cyan()
    );
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "[7]  ⚔️  Live Battle Demo (Offense vs Defense)"
            .bright_magenta()
            .bold(),
        "      │".bright_cyan()
    );
...
fn print_method_menu(capacity: u64, interface: Option<&str>, device_type: Option<&DeviceType>) {
    ...
    // Hardware Firmware Purge only makes sense on SSD/NVMe controllers that
    // might actually honor a DSM TRIM/Sanitize command. USB flash drives,
    // SD cards, eMMC/UFS, and anything behind a plain USB bridge essentially
    // never expose this passthrough — offering it there just leads to a
    // no-op that produces a misleading "purged" impression. Gate it out
    // instead of letting it fail silently after the fact.
    let offer_hardware_purge = matches!(
        device_type,
        Some(DeviceType::InternalSsd) | Some(DeviceType::InternalNvme) | Some(DeviceType::ExternalSsd)
    );

    if offer_hardware_purge {
        println!(
            "\n    {} {:<38} {} {}",
    ...
    } else if device_type.is_some() {
        println!(
            "\n    {}",
            "ℹ Hardware Firmware Purge not offered for this device — USB/flash-bridged \
             media rarely supports hardware erase passthrough; use an overwrite method above."
                .dimmed()
        );
    }
...
fn print_settings_info(groq_client: &Option<ai::groq::GroqClient>) {
    ...
    println!(
        "    {:<20} {}",
        "Standards:".dimmed(),
        format!("{} methods available", SanitizeMethod::all_methods().len())
    );
    println!(
        "    {:<20} {}",
        "AI Features:".dimmed(),
        if groq_client.is_some() {
            "Enabled (Groq API)".green().to_string()
        } else {
            "Disabled (set GROQ_API_KEY)".yellow().to_string()
        }
    );
    println!(
        "    {:<20} {}",
        "Hot-Plug:".dimmed(),
        "Active (monitoring USB events)".green()
    );
    println!(
        "    {:<20} {}",
        "Platform:".dimmed(),
        "Windows (Win32 Raw I/O)"
    );
}
...
fn cmd_erase_interactive(
    ...
    audit.log(
        ...
        "PS149 sanitization session started",
    );
    ...
    let eligible: Vec<&PhysicalDisk> = disks
        .iter()
        .filter(|d| d.safety_status.is_erasable())
        .collect();
    ...
    print_method_menu(target.capacity, target.interface_type.as_deref(), Some(&target.device_type));
    let mut method_input = read_input("Select method")?;
    ...
    if method_input.trim().eq_ignore_ascii_case("p") {
        println!(
            "\n  {}",
            "Hardware Firmware Purge (NIST SP 800-88 Purge)"
                .bright_magenta()
                .bold()
        );
        println!(
            "  Target: Disk {} ({})",
            target.index,
            target.model.as_deref().unwrap_or("Unknown")
        );
        ...
        println!(
            "    Status   : {}",
            if purge_result.verified_purge {
                "VERIFIED PURGE — NVMe Sanitize Crypto Erase completed".bright_green().bold()
            } else if purge_result.success {
                "TRIM/DEALLOCATE ACKNOWLEDGED (best-effort — not a verified Purge)"
                    .bright_green()
                    .bold()
            } else {
                "NOT SUPPORTED — no data was erased".red().bold()
            }
        );
        ...
        if purge_result.success {
            // Append to blockchain & BSA 2023 certificate — only when the
            // controller actually acknowledged the command. A failed/no-op
            // attempt must never produce a report that looks like a
            // completed sanitization (see the false-success bug this fixes).
            let mut chain = report::blockchain::AuditChain::load_or_create().unwrap_or_default();
            let serial = target
                .serial_number
                .clone()
                .unwrap_or_else(|| format!("Disk_{}", target.index));
            let _ = chain.add_event(
                report::blockchain::AuditEventType::DriveErasure,
                &serial,
                "Forensic_Operator",
                if purge_result.verified_purge {
                    "NVME_SANITIZE_CRYPTO_ERASE_VERIFIED"
                } else {
                    "HARDWARE_TRIM_DEALLOCATE_ACKNOWLEDGED"
                },
                &format!(
                    "Hardware Purge: {} on Disk {}",
                    purge_result.command_type, target.index
                ),
                None,
            );

            // NOTE: a bare TRIM/Deallocate is advisory only — the controller
            // decides when (or whether) the underlying NAND actually gets
            // erased. NVMe Sanitize via IOCTL_STORAGE_REINITIALIZE_MEDIA is
            // different: that call blocks until completion, so a success
            // there is a genuine verified erase — the wording below reflects
            // whichever mechanism actually ran (`purge_result.verified_purge`),
            // never overclaiming TRIM as "100% purged."
            let bsa_cert = if purge_result.verified_purge {
                report::blockchain::generate_bsa_certificate(
                    "NVMe Sanitize Crypto Erase — Verified NIST SP 800-88 Purge",
                    &format!(
                        "Disk {} ({})",
                        target.index,
                        target.model.as_deref().unwrap_or("Drive")
                    ),
                    &serial,
                    "Hardware Controller NVMe Sanitize (IOCTL_STORAGE_REINITIALIZE_MEDIA, Crypto Erase)",
                    "N/A",
                    "SANITIZE_COMPLETED_VERIFIED",
                    "Drive controller confirmed completion of the Sanitize command before this \
                     tool returned — the media encryption key was invalidated, rendering all \
                     previously written data cryptographically unrecoverable.",
                    chain.merkle_root.as_deref().unwrap_or("GENESIS"),
                    chain.blockchain_tx.as_deref(),
                )
            } else {
                report::blockchain::generate_bsa_certificate(
                    "Hardware TRIM/Deallocate (Best-Effort — Not a Verified NIST 800-88 Purge)",
                    &format!(
                        "Disk {} ({})",
                        target.index,
                        target.model.as_deref().unwrap_or("Drive")
                    ...
                    &serial,
                    "Hardware Controller TRIM/Deallocate (DSM)",
                    "N/A",
                    "TRIM_ACKNOWLEDGED_NOT_VERIFIED",
                    "Controller acknowledged the deallocate command. Actual physical erasure \
                     timing and completeness are NOT guaranteed by this command alone — treat \
                     as best-effort flash hygiene, not a compliance-grade Purge.",
                    chain.merkle_root.as_deref().unwrap_or("GENESIS"),
                    chain.blockchain_tx.as_deref(),
                )
            };
            ...
            let bsa_path = report_dir.join(format!(
                "bsa_section_63_hardware_purge_{}.txt",
                timestamp_str
            ));
            let _ = std::fs::write(&bsa_path, &bsa_cert);
            println!(
                "    BSA 2023 : {}",
                bsa_path.display().to_string().bright_yellow().bold()
            );
        ...
        // Failure: nothing was erased, and no certificate/blockchain entry
        // is generated for it — fall back into a real overwrite pass rather
        // than leaving the drive untouched while the user thinks it's done.
        audit.log(
            AuditEventType::Error,
            format!(
                "Hardware purge unsupported on Disk {} ({}): {}",
                target.index, purge_result.command_type, purge_result.message
            ),
        );
        println!(
            "\n  {}",
            "No data was erased by the hardware purge attempt. Falling back to a \
             pattern-overwrite method — select one below."
                .yellow()
                .bold()
        );
        print_method_menu(target.capacity, target.interface_type.as_deref(), Some(&target.device_type));
        method_input = read_input("Select overwrite method")?;
        if method_input.trim() == "0" {
            return Ok(());
        }
    }
    ...
    audit.log(
        ...
        format!("Device safety status: {}", target.safety_status),
    );
    ...
    audit.log(
        AuditEventType::ConfirmationReceived,
        "User confirmed erasure",
    );
    ...
    let is_zone_method = matches!(
        method,
        SanitizeMethod::FastWipe | SanitizeMethod::SmartSecure
    );
    ...
    let sanitize_result = sanitize::execute_sanitization(&target, method, |progress| {
        ...
        pb.set_position(
            (progress.pass_index as u64 * total_bytes / method.pass_count() as u64) + bytes_done,
        );
    ...
    let last_pattern =
        sanitize::patterns::get_pattern(method.pass_count().saturating_sub(1), method);
    ...
    if let Some(ref client) = groq_client {
        ...
        match ai::report_narrator::generate_narrative(
            ...
            Err(e) => {
                println!(
                    "  {}",
                    format!("✗ Failed to generate narrative: {}", e).yellow()
                );
            }
        }
    }
    ...
    if verify_result.passed {
        println!(
            "  {}",
            "✓ SANITIZATION COMPLETE — VERIFICATION PASSED"
                .green()
                .bold()
        );
    } else {
    ...
    let desc = format!(
        "Erase disk {} with {}",
        target.index, cert.sanitization.method
    );
    let _ = chain.add_event(
        report::blockchain::AuditEventType::DriveErasure,
        &serial,
    ...
    println!(
        "\n  {}",
        "Reports & Evidence Certificates saved:"
            .bold()
            .bright_green()
    );
    println!(
    ...
    println!(
        "    Text Certificate : {}",
        txt_path.display().to_string().cyan()
    );
    println!(
        "    BSA 2023 Sec 63  : {}",
        bsa_path.display().to_string().bright_yellow().bold()
    ...
    println!(
        "  {}",
        "The drive's partition table has been erased (RAW state).".dimmed()
    );
    println!(
        "  {}",
        "Would you like to initialize and format it for immediate reuse?".white()
    );
    println!();
    println!(
        "    {} FAT32  — Universal compatibility (Windows, Mac, Linux, TV, Car)",
        "[1]".bright_green()
    );
    println!(
        "    {} exFAT  — Modern cross-platform, supports files >4GB",
        "[2]".cyan()
    );
    println!(
        "    {} NTFS   — Windows optimized with journaling",
        "[3]".yellow()
    );
    println!(
        "    {} Leave as RAW (Forensic unallocated proof)",
        "[0]".dimmed()
    );
    ...
    if let Some(fs_type) = target_fs {
        println!(
            "\n  {}",
            format!(
                "Initializing Disk {} with {}...",
                target.index,
                fs_type.as_str().to_uppercase()
            )
            .cyan()
        );
        match sanitize::initialize::reinitialize_and_format_disk(target.index, fs_type, "CLEAN_USB")
        {
            Ok(res) => {
                println!(
                    "  {}",
                    format!("✓ {}", res.output_summary).bright_green().bold()
                );
                println!(
                    "  {}",
                    "Drive is now clean and immediately ready to use in Windows Explorer!".green()
                );
            }
            Err(e) => {
                println!(
                    "  {}",
                    format!("✗ Formatting encountered an issue: {}", e).yellow()
                );
                println!(
                    "  {}",
                    "You can format manually through Windows Disk Management.".dimmed()
                );
            }
        }
    } else {
        println!(
            "  {}",
            "Drive left in raw, unpartitioned state for forensic verification.".dimmed()
        );
    }
...
fn print_file_erase_menu() {
    ...
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "Module 2: Secure File & Folder Shredder"
            .bold()
            .bright_white(),
        "          │".bright_cyan()
    );
    ...
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "[1]  Shred a Specific File".white(),
        "                             │".bright_cyan()
    );
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "[2]  Shred a Specific Folder (Recursive)".white(),
        "                 │".bright_cyan()
    );
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "[3]  Batch Shred Multiple Files / Folders".white(),
        "                │".bright_cyan()
    );
    println!(
        "  {}  {}  {}",
        "│".bright_cyan(),
        "[4]  Wipe Free Space on a Volume (Anti-Carving)".white(),
        "          │".bright_cyan()
    );
    println!(
        "  {}  {}  {}",
...
fn select_shred_method(file_size: u64) -> Result<SanitizeMethod> {
    print_method_menu(file_size, None, None);
...
fn confirm_shred(target_name: &str) -> Result<bool> {
    ...
    println!(
        "  {}",
        "This action will overwrite data, purge Alternate Data Streams, wipe slack space,".dimmed()
    );
    println!(
        "  {}",
        "and scramble MFT metadata records. Reconstruction will be impossible.".dimmed()
    );
...
fn shred_single_file(groq_client: &Option<ai::groq::GroqClient>) -> Result<()> {
    ...
    println!(
        "\n  {}",
        format!(
            "Executing Forensic Shred: {} ({} passes)...",
            method.display_name(),
            method.pass_count()
        )
        .cyan()
    ...
    // ── Post-shred: Purge NTFS journal traces ──
    if let Some(vol) = file_eraser::vss::volume_letter_from_path(&path) {
        println!("  {}", "Purging NTFS USN journal traces...".dimmed());
        match file_eraser::journal::purge_usn_journal(vol) {
            Ok(_) => println!("  {}", "✓ USN journal purged".green()),
            Err(e) => println!("  {}", format!("⚠ USN purge skipped (need admin): {}", e).yellow()),
        }
    }

    // ── Post-shred: Flood $LogFile to overflow NTFS transaction records ──
    if let Some(vol) = file_eraser::vss::volume_letter_from_path(&path) {
        println!("  {}", "Flooding $LogFile to destroy transaction records...".dimmed());
        match file_eraser::logfile::flood_logfile(vol, 10_000, &|_msg, _pct| {
            // Silent — don't spam for 10k file ops
        }) {
            Ok(r) => println!("  {}", format!("✓ $LogFile flooded: {} operations", r.operations_generated).green()),
            Err(e) => println!("  {}", format!("⚠ $LogFile flood: {}", e).yellow()),
        }
    }

    // ── Post-shred: Clean Windows system traces ──
    println!("  {}", "Cleaning Windows system artifact traces...".dimmed());
    let clean_targets = vec![path.clone()];
    match system_cleaner::clean_system_traces(
        &clean_targets,
        &system_cleaner::CleanOptions::default(),
        &|msg, _pct| { println!("    {}", msg.dimmed()); },
    ) {
        Ok(report) => {
            let total = report.prefetch_cleaned + report.thumbcache_cleaned
                + report.recent_docs_cleaned + report.recycle_bin_cleaned
                + report.extended_cleaned;
            if total > 0 {
                println!("  {}", format!("✓ {} system artifacts cleaned", total).green());
            }
        }
        Err(e) => println!("  {}", format!("⚠ System trace clean: {}", e).yellow()),
    }
    ...
    if let Some(ref client) = groq_client {
        ...
        match ai::file_narrator::generate_file_narrative(
            client,
            &res,
            &start_time.to_rfc3339(),
            &end_time.to_rfc3339(),
    ...
    if res.success() {
        println!(
            "  {}",
            "✓ FILE PERMANENTLY DESTROYED & MFT PURGED".green().bold()
        );
    } else {
        println!(
            "  {}",
            "⚠️ FILE ERASURE COMPLETED WITH NOTED WARNINGS"
                .yellow()
                .bold()
        );
    }
    ...
    let cert = FileErasureCertificate {
        ...
        compliance_note: "Forensic shred completed. Physical cluster data overwritten, \
            ...
            entries scrambled via multi-iteration rename cycles."
            .to_string(),
    };
...
fn shred_folder(groq_client: &Option<ai::groq::GroqClient>) -> Result<()> {
    ...
    if total_files == 0 {
        println!(
            "  {}",
            "Folder contains no files. Removing directory...".dimmed()
        );
    ...
    println!(
        "\n  {}",
        format!(
            "Executing Recursive Shred: {} files with {}...",
            total_files,
            method.display_name()
        )
        .cyan()
    ...
    // ── Post-shred: Flood $LogFile ──
    if let Some(vol) = file_eraser::vss::volume_letter_from_path(&path) {
        println!("  {}", "Flooding $LogFile to destroy transaction records...".dimmed());
        match file_eraser::logfile::flood_logfile(vol, 10_000, &|_msg, _pct| {}) {
            Ok(r) => println!("  {}", format!("✓ $LogFile flooded: {} ops", r.operations_generated).green()),
            Err(e) => println!("  {}", format!("⚠ $LogFile flood: {}", e).yellow()),
        }
    }
    ...
    let cert = FileErasureCertificate {
        ...
        ai_narrative,
        compliance_note:
            "Folder destruction complete. All child files and directories neutralized.".to_string(),
    };
...
fn shred_batch(groq_client: &Option<ai::groq::GroqClient>) -> Result<()> {
    ...
    println!(
        "  {}",
        "Enter paths separated by semicolon ';' (e.g. C:\\secret.doc; D:\\archive; E:\\file.bin)"
            .dimmed()
    );
    ...
    // ── Post-shred: Purge NTFS journal for each affected volume ──
    {
        let mut purged_volumes = std::collections::HashSet::new();
        for t in &target_paths {
            if let Some(vol) = file_eraser::vss::volume_letter_from_path(t) {
                if purged_volumes.insert(vol) {
                    println!("  {}", format!("Purging USN journal on volume {}:...", vol).dimmed());
                    match file_eraser::journal::purge_usn_journal(vol) {
                        Ok(_) => println!("  {}", format!("✓ USN journal purged on {}:", vol).green()),
                        Err(e) => println!("  {}", format!("⚠ USN purge on {}: {}", vol, e).yellow()),
                    }
                }
            }
        }
    ...
    // ── Post-shred: Flood $LogFile on each affected volume ──
    {
        let mut flooded_volumes = std::collections::HashSet::new();
        for t in &target_paths {
            if let Some(vol) = file_eraser::vss::volume_letter_from_path(t) {
                if flooded_volumes.insert(vol) {
                    println!("  {}", format!("Flooding $LogFile on volume {}:...", vol).dimmed());
                    match file_eraser::logfile::flood_logfile(vol, 10_000, &|_msg, _pct| {}) {
                        Ok(r) => println!("  {}", format!("✓ $LogFile flooded on {}: {} ops", vol, r.operations_generated).green()),
                        Err(e) => println!("  {}", format!("⚠ $LogFile flood on {}: {}", vol, e).yellow()),
                    }
                }
            }
        }
...
fn wipe_volume_free_space(_groq_client: &Option<ai::groq::GroqClient>) -> Result<()> {
    ...
    println!(
        "  {}",
        "Overwrites unallocated clusters where previously deleted files reside.".dimmed()
    );
    ...
    let letter = input
        ...
        .chars()
        .next()
        .context("No drive letter provided")?
        .to_ascii_uppercase();

    if letter == 'C' {
        println!(
            "\n  {}",
            "⚠️  WARNING: Wiping free space on system drive C: can impact running programs."
                .yellow()
                .bold()
        );
    ...
    println!(
        "  Using {} standard for free space wipe.",
        method.display_name().cyan()
    );
    ...
    println!(
        "  {}",
        format!(
            "✓ FREE SPACE WIPED ON {}:\\ ({} MB purged)",
            letter,
            res.bytes_wiped / 1_048_576
        )
        .green()
        .bold()
    );
    ...
    let cert = FileErasureCertificate {
        ...
        ai_narrative: None,
        compliance_note: "Volume unallocated cluster purge complete via temporary allocation fill."
            .to_string(),
    };
...
fn cmd_view_history() -> Result<()> {
    ...
    println!(
        ...
        chain.summary().bright_white().bold()
    );
    println!("  {}", "─".repeat(60).dimmed());
    ...
    println!();
    println!(
        "    {} Verify Blockchain Hash Chain & Merkle Integrity",
        "[V]".bright_magenta().bold()
    );
    println!(
        "    {} Inspect Raw Blockchain Ledger (All Blocks)",
        "[B]".cyan().bold()
    );
    ...
    match choice_trimmed {
        ...
        "v" | "V" => {
            println!(
                "\n  {} Verifying Blockchain Audit Chain...",
                "🔍".to_string().bright_magenta()
            );
            ...
            if valid {
                println!(
                    ...
                    "✓".bright_green().bold(),
                    "PERFECT — 0 TAMPERING DETECTED".bright_green().bold()
                );
                ...
                println!(
                    "  {} Blockchain Chain Integrity: {}",
                    "✗".red().bold(),
                    "CORRUPTED OR TAMPERED!".red().bold()
                );
        ...
        "b" | "B" => {
            ...
            if chain.entries.is_empty() {
                ...
                for entry in &chain.entries {
                    ...
                    println!("      Prev Hash : {}...", &entry.prev_hash[..16].dimmed(),);
                    println!(
                        "      Block Hash: {}...",
...
fn cmd_file_recovery_interactive(groq: &Option<ai::groq::GroqClient>) -> Result<()> {
    ...
    println!(
        "  {}  {}  {}",
        "│".bright_yellow(),
        "[1] Carve from Physical Drive".white(),
        "                    │".bright_yellow()
    );
    println!(
        "  {}  {}  {}",
        "│".bright_yellow(),
        "[2] Carve from Volume (D:, E:, ...)".white(),
        "              │".bright_yellow()
    );
    println!(
        "  {}  {}  {}",
        "│".bright_yellow(),
        "[3] Carve from Disk Image (.dd, .raw, .img)".white(),
        "     │".bright_yellow()
    );
    println!(
        "  {}  {}  {}",
    ...
    let (source_path, source_size, source_display) = match choice.trim() {
        "1" => {
            ...
            if disk.is_boot_disk {
                println!(
                    "  {} Cannot carve from boot disk while Windows is running.",
                    "⚠".red().bold()
                );
    ...
    println!(
        ...
        "│".bright_yellow(),
        "Size".bold(),
        model::device::format_bytes(source_size).yellow(),
        "│".bright_yellow()
    );
    println!(
        "  {}  {}: {}  {}",
        "│".bright_yellow(),
        "Signatures".bold(),
        format!("{} file types", carver::all_signatures().len()).cyan(),
        "│".bright_yellow()
    );
    println!(
        "  {}",
    ...
    println!(
        "\n  {} File carving will {}.",
        "⚠".yellow().bold(),
        "READ the source (non-destructive)".bright_green().bold()
    );
    println!(
        "    Recovered files will be saved to: {}",
    ...
    pb.set_style(
        indicatif::ProgressStyle::default_bar()
            .template(
                "  Scan [{bar:40.yellow/dim}]  {percent}% | {bytes}/{total_bytes} | Files: {msg}",
            )
            .unwrap()
            .progress_chars("█▓░"),
    );
    ...
    let result =
        carver::carve_from_source(&source_path, &output_dir, Some(source_size), |progress| {
    ...
    println!(
        "  {} {}",
        "CARVING RESULTS".bold().bright_yellow(),
        "═".repeat(43).bright_yellow()
    );
    ...
    println!(
        "\n  {} Files Recovered: {}",
        "📁".to_string(),
        result.files_found.to_string().bright_green().bold()
    );
    println!(
        "  {} Data Scanned: {}",
        "💾".to_string(),
        model::device::format_bytes(result.total_bytes_scanned).cyan()
    );
    println!(
        "  {} Duration: {:.1}s",
        "⏱".to_string(),
    ...
    if !result.carved_files.is_empty() {
        ...
        println!(
            "\n  {} {}",
            "ℹ".bright_cyan(),
            "No recoverable files found. The drive may have been securely wiped.".dimmed()
        );
    }
    ...
    let _ = chain.add_event(
        ...
        &format!(
            "Carved {} files from {}",
            result.files_found, source_display
        ),
        None,
    );
    ...
    println!(
        "    BSA 2023 Sec 63  : {}",
        bsa_carve_path.display().to_string().bright_yellow().bold()
    );
    println!(
        "    Blockchain Audit : {}",
        chain.summary().bright_magenta()
    ...
    if let Some(client) = groq {
        println!(
            "\n  {} Generating AI forensic narrative...",
    ...
    println!(
        "\n  {} Output directory: {}",
        "📂".to_string(),
        output_dir.display().to_string().bright_green().bold()
    );
