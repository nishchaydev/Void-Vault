...
pub struct AuditEntry {
    ...
    /// Shannon entropy of the post-wipe sector sample (0.0 = all zeros, 8.0 = perfect random).
    /// Proves to auditors that data was actually destroyed, not just "claimed" to be.
...
impl AuditChain {
    ...
    /// Returns the sibling hashes needed to verify the entry against the root.
    /// An auditor can verify a single entry in O(log N) without the full chain.
    pub fn merkle_proof(&self, entry_index: usize) -> Option<Vec<(String, bool)>> {
        ...
        while hashes.len() > 1 {
            ...
            for chunk in hashes.chunks(2) {
                ...
                if idx == chunk_start {
                    proof.push((right.clone(), true)); // sibling is on the right
                    next_idx = next_level.len();
                } else if idx == chunk_start + 1 {
                    proof.push((left.clone(), false)); // sibling is on the left
                    next_idx = next_level.len();
                }
...
fn compute_entry_hash(
    ...
    if let Some(e) = entropy {
        hasher.update(format!("{:.6}", e).as_bytes());
    }
...
╚══════════════════════════════════════════════════════════════════════╝"#,
        ...
        pre_hash = if pre_hash.len() > 40 {
            &pre_hash[..40]
        } else {
            pre_hash
        },
        post_hash = if post_hash.len() > 40 {
            &post_hash[..40]
        } else {
            post_hash
        },
        verification = verification_result,
        merkle = if merkle_root.len() > 40 {
            &merkle_root[..40]
        } else {
            merkle_root
        },
