...
use windows::Win32::Storage::FileSystem::{
    CreateFileW, FlushFileBuffers, GetFileSizeEx, SetEndOfFile, SetFilePointerEx, WriteFile, FILE_BEGIN,
    FILE_FLAGS_AND_ATTRIBUTES, FILE_FLAG_WRITE_THROUGH, FILE_SHARE_NONE, OPEN_EXISTING,
};
...
pub fn wipe_file_slack(path: &Path) -> Result<u64> {
    ...
    unsafe {
        ...
        // Restore original file size — the write above extended the logical
        // file length. We must truncate back to prevent corrupting the file.
        SetFilePointerEx(handle, file_size as i64, None, FILE_BEGIN)?;
        SetEndOfFile(handle)?;
    }
...
fn get_cluster_size_for_file(path: &Path) -> Option<u64> {
    ...
    let volume_root = if path_str.starts_with(r"\\?\") && path_str.len() >= 6 && path_str.as_bytes()[5] == b':' {
        format!("{}\\", &path_str[4..6])
    ...
    match ok {
        Ok(_) => {
            ...
            info!(
                "Volume {} cluster size: {} bytes",
                volume_root, cluster_size
            );
            Some(cluster_size)
        }
        Err(e) => {
