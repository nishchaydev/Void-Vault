...
///
/// ⚠️ SECURITY WARNING: This client sends forensic metadata to a US-based
/// cloud API (api.groq.com). For defense/government/classified investigations,
/// this feature MUST be disabled. Set GROQ_API_KEY to empty to disable.
/// For air-gapped environments, consider running LLaMA locally via llama.cpp.
...
impl GroqClient {
    ...
    pub fn from_env() -> Option<Self> {
        let api_key = std::env::var("GROQ_API_KEY").ok().or_else(|| {
        ...
        tracing::warn!(
            "⚠️ AI narrative features ENABLED — forensic metadata will be sent to \
             api.groq.com (US cloud). Disable by clearing GROQ_API_KEY for classified work."
        );
        Some(Self {
            api_key,
