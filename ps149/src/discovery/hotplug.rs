    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::DeviceInserted {
                ...
                write!(
                    f,
                    "NEW DEVICE: {} ({:.1} GB) on {} as Disk {}",
                    model, cap_gb, interface, disk_index
                )
            }
...
fn watch_loop(tx: &mpsc::Sender<HotPlugEvent>) -> anyhow::Result<()> {
    ...
    loop {
        ...
        let current: Vec<std::collections::HashMap<String, wmi::Variant>> = match wmi_con
            .raw_query("SELECT Index, Model, Size, InterfaceType FROM Win32_DiskDrive")
        {
            Ok(v) => v,
            Err(_) => continue,
        };
